...
describe('FlightPlan', () => {
  test('can be created', () => {
    ...
    // When + Then
    expect(
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, services, phases)
    ).not.toThrow()
  })
  ...
  test('needs an ID', () => {
    ...
    expect(
      ...
      () => new FlightPlan(null, name, branch, environment, services, phases)
  ...
  test('needs a name', () => {
    ...
    expect(
      ...
      () => new FlightPlan(crypto.randomUUID(), null, branch, environment, services, phases)
    ).toThrow(expectedError)
  })
  ...
  test('needs a target workflow branch name', () => {
    ...
    expect(
      ...
      () => new FlightPlan(crypto.randomUUID(), name, null, environment, services, phases)
    ).toThrow(expectedError)
  })
  ...
  test('needs a target environment', () => {
    ...
    expect(() => new FlightPlan(crypto.randomUUID(), name, branch, null, services, phases)).toThrow(
      expectedError
    )
  })
  ...
  test('needs valid target service IDs', () => {
    ...
    expect(
      ...
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, null, phases)
    ).toThrow(expectedError)
    expect(
      // @ts-expect-error A Flight Plan needs valid target service IDs.
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, 'test', phases)
    ).toThrow(expectedError)
    expect(
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, [], phases)
    ).toThrow(expectedError)
  })
  ...
  test('needs valid phases', () => {
    ...
    expect(
      ...
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, services, null)
    ).toThrow(expectedError)
    expect(
      // @ts-expect-error A Flight Plan needs valid phases.
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, services, 'test')
    ).toThrow(expectedError)
    expect(
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, services, [])
    ).toThrow(expectedError)
  })
  ...
  test('needs valid workflow inputs', () => {
    ...
    expect(
      ...
      () => new FlightPlan(crypto.randomUUID(), name, branch, environment, services, phases, 'test')
    ).toThrow(expectedError)
  })
})
